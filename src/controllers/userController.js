...
const authorByAge = async function (req, res) {
    ...
    let booklist = await bookModel.find({ ratings: { $gt: 4 } }).select({ author_id: 1, _id: 0 })
    let arr = booklist.author_id
    let authors = await authorModel.find({ $and: [{ author_id: booklist.author_id }, { age: { $gt: 50 } }] })
